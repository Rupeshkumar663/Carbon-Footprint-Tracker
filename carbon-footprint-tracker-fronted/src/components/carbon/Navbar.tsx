  ...
  return (
    <nav className="flex justify-between items-center px-4 sm:px-6 lg:px-8 py-5 bg-black/80 shadow-sm relative z-50">
      ...
      <div className="hidden md:flex items-center gap-10">
        ...
        {(variant==="inputpage"||variant==="carbonemissionpage") && <span onClick={()=>navigate("/dashboard")} className="text-green-400 text-xl font-bold hover:text-green-300 cursor-pointer">Vehicledashboard</span>}
         
        {variant==="inputpage"&& <span onClick={()=>navigate("/flighthome")} className="text-green-400 text-xl font-bold hover:text-green-300 cursor-pointer">FlightInput</span>}

        {variant==="inputpage"&& <span onClick={()=>navigate("/fighter-jet")} className="text-green-400 text-xl font-bold hover:text-green-300 cursor-pointer">FighterjetInput</span>}
        
        {variant==="carbonemissionpage" && <span onClick={()=>navigate("/inputpage")} className="text-green-400 text-xl font-bold hover:text-green-300 cursor-pointer">VehicleInput</span>}
      ...
      <AnimatePresence>
        {menuOpen && (
         ...
         {variant==="carbonemissionpage" && (
          <span onClick={()=>{navigate("/inputpage"); setMenuOpen(false);}}className="cursor-pointer hover:text-green-300">VehicleInput</span>)}
