...
const TodayCarbonCard:React.FC=()=>{
  ...
  const fetchTodayCarbon=async()=>{
    try {
      ...
      const todayData=records.filter((item:{createdAt:string})=>{
        const date=new Date(item.createdAt);
        return date>=today;
      });
      const total=todayData.reduce((sum:number,item:{carbonEmission:number})=>sum+(item.carbonEmission || 0),0);
 ...
 const formatCarbon=(value:number)=>{
  ...
  return (
    <div className="relative px-4 sm:px-8 py-6 sm:py-10 rounded-2xl text-center bg-black backdrop-blur-2xl border border-green-500/20 overflow-hidden">
      ...
      <h2 className="text-green-300 text-2xl sm:text-3xl font-bold tracking-wide">{formatCarbon(todayCarbon)} kg CO₂</h2>
    </div>
  );
};
