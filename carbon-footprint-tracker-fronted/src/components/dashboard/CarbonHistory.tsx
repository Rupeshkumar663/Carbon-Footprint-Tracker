...
const CarbonHistory:React.FC=()=>{
  ...
  return (
    ...
    <motion.div
      ...
      className="w-full h-[320px] sm:h-[340px] md:h-[380px] p-3 sm:p-4 rounded-xl bg-black/90 backdrop-blur-xl border border-white/10 shadow-lg flex flex-col">
      <div className="mb-2">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1 scroll-smooth">
        ...
        {!loading && data.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <p className="text-gray-500 text-xs mt-2max-w-[260px] leading-relaxed">
        ...
        {!loading &&data.map((item)=>{
            ...
            return (
              <div
                key={item._id} className="flex justify-between items-start px-3 py-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 transition-all duration-300">
                <div className="min-w-0">
                  <p className="text-green-300 text-[10px] sm:text-xs mt-1 break-words">{item.vehicle_name}</p>
                  <p className="text-green-400 text-xs sm:text-sm font-medium break-words">{item.startLocation} → {item.endLocation}</p>
                ...
                <div className="text-right ml-3 flex-shrink-0">
                  ...
                  <p className="text-green-200 text-[11px] sm:text-xs mt-1">CO₂</p>
                </div>
              </div>
            );
          })}
