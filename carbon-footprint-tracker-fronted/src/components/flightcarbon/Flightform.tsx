    ...
    <div className="bg-gradient-to-b from-[#0b2a18] to-[#061a0d] p-6 rounded-xl border border-green-900">
      <div className="mb-6">
        ...
        <div className="flex gap-6">
          <button onClick={()=>setTripType("round")}
            className={`px-4 py-2 rounded-full border ${
              tripType === "round"? "bg-green-600 border-green-500" : "border-gray-600"}`}>Round trip
          </button>
          <button onClick={() => setTripType("oneway")} className={`px-4 py-2 rounded-full border ${ tripType === "oneway" ? "bg-green-600 border-green-500" : "border-gray-600"}`}> One-way</button>
        </div>
      </div>
          ...
          {["Economy", "Premium Economy", "Business", "First Class"].map(
            (item) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border ${
                  cabin === item ? "bg-green-600 border-green-500" : "border-gray-600"
                }`}>{item}
              </button>
            )
          )}
      ...
      <button className="w-full bg-green-600 hover:bg-green-700 transition p-3 rounded-md font-semibold"> Calculate CO₂ emission</button>
      <div className="mt-6 p-4 border border-green-900 rounded-lg text-gray-400 text-sm">
        <p className="mb-2 text-white font-medium"> Before booking your flight</p>
