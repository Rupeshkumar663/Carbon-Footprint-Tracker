export default function GlobalImpact({total,impact}:{
  ...
  return (
    <div className="w-full bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-6 text-center shadow-lg">
      ...
      <p className="text-green-300 text-xs mb-4">of global daily emissions</p>
      <div className="grid grid-cols-2 gap-3 text-xs sm:text-sm mt-3">
        <div className="bg-white/5 p-3 rounded-xl text-green-200"> {impact.trees.toLocaleString()}
          <div className="text-green-400 text-xs">Trees</div>
        </div>
        <div className="bg-white/5 p-3 rounded-xl text-green-200"> {impact.jetFuel.toLocaleString()}
          <div className="text-green-400 text-xs">Fuel (L)</div>
        </div>
        <div className="bg-white/5 p-3 rounded-xl text-green-200"> {impact.flightHours}
          <div className="text-green-400 text-xs">Hours</div>
        </div>
        <div className="bg-white/5 p-3 rounded-xl text-green-200"> {impact.earthTrips}
          <div className="text-green-400 text-xs">Earth Trips</div>
        </div>
      </div>
    </div>
  );
