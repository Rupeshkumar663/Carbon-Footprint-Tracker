     ...
     className="w-full h-[320px] sm:h-[380px] p-4 rounded-2xl bg-black/90 border border-white/10 shadow-xl flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1">
        ...
        {!loading && data.length === 0 && (
          <p className="text-center text-gray-400 text-sm py-8">No fighter jet mission history available.</p>
        )}
            ...
            return (
              <div
                ...
                <div className="flex flex-col">
                  <p className="text-green-300 text-sm font-semibold break-words"> {item.jetModel || "Unknown Jet"} • {item.mission || "No Mission"}</p>
                  <p className="text-gray-400 text-xs">{date} • {time}</p>
                </div>
                <div className="text-right">
                  <p className={`text-sm font-semibold ${getColor(item.totalCO2)}`}>{Math.round(item.totalCO2 || 0).toLocaleString()} kg</p>
