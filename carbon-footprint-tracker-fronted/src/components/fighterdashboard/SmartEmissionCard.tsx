...
const SmartEmissionCard:React.FC=()=>{
  ...
  useEffect(()=>{
    const fetchData=async()=>{
      try {
        const res=await api.get("/api/fighter/fighterjetsmartemission");
 ...
 if(loading){
   return (
     <div className="bg-black/90 p-6 rounded-2xl border border-white/10 text-center">
 ...
 if(!data){
  ...
  return (
    <motion.div
      ...
      <h2 className="text-3xl sm:text-4xl md:text-5xl text-green-300 font-bold break-words">{Math.round(data?.totalCO2).toLocaleString()} kg CO₂</h2>
      <div className="mt-3">
      ...
      <p className="text-green-400 text-sm mt-3"> {data.trees} Trees equivalent</p>
