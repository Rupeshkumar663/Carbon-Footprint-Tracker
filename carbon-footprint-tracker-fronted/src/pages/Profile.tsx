...
function Profile(){
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-3 sm:px-4 py-6 sm:py-10 flex items-center justify-center">
      <div className="bg-white shadow-lg rounded-2xl p-4 sm:p-6 max-w-md w-full relative">
        <FaArrowLeftLong className="absolute top-5 left-5 w-[22px] h-[22px] cursor-pointer text-green-700" onClick={()=>navigate("/")}/>
        <div className="flex flex-col items-center text-center">
          {userData.photoUrl ? (
            <img src={userData.photoUrl} className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover border-4 border-green-800" alt="Profile"  onError={(e)=> {e.currentTarget.style.display="none";}}/>
          ) : (
          ...
          <h2 className="text-xl sm:text-2xl font-bold mt-4 text-green-600">{userData.name}</h2>
        ...
        <div className="mt-6 flex justify-center gap-4">
          <button
           className="w-full sm:w-auto px-4 py-2 border border-green-400 text-green-400 rounded-lg font-medium hover:bg-green-400 hover:text-black transition" onClick={() => navigate("/editprofile")} > Edit Profile
          </button>
        </div>
      </div>
    </div>
